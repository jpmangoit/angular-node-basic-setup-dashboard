...
export class EditUserComponent implements OnInit {
    ...
    getUser() {
        const endPoint: string = '/admin-user/get-user/' + this.userId;
        this._authService.setLoader(true);
        this._authService.sendRequest('get', endPoint, '').subscribe((respData: any) => {
            this._authService.setLoader(false);
    ...
    getRole() {
        ...
        this._authService.sendRequest('get', endPoint, '').subscribe((respData: any) => {
            this._authService.setLoader(false);
            if (respData?.isError == false) {
                this.userRoles = respData.result.data
            }
        }, (err) => {
            this._authService.setLoader(false);
            this.toastrService.showError(err.message, 'Error');
    ...
    setValue(formValue: any) {     
        ...
        this.userForm.controls["address"].setValue(formValue.address);
        this.userForm.controls["companyName"].setValue(formValue.companyName);
    ...
    onSubmit() {
        ...
        if (this.userForm.invalid || this.errorImage.isError || this.errorIcon.isError) {
            ...
            const endPoint: string = '/admin-user/update-user/' + this.userId;
            this._authService.setLoader(true);
