...
export class DashboardComponent {
    ...
    orderData: any;
    allOrderData: any;
    endPointAmazon: string = '/user-amazon/get-amazon-data'
    endPointWalmart: string = '/user-walmart/get-walmart-data'
    items: any;
    totalRows: number = 0;
    pageSize: number = 10;
    currentPage: any = 0;
    ...
    getAmazonData() {
        ...
        let data = {
            ...
            // api_path: `/listings/2021-08-01/items/A3OFY3HQ6B3HO5/0B-GCBH-RNP0`,
            // method: 'GET',
            // query: {
            //     marketplaceIds: ["ATVPDKIKX0DER"],
            //     includedData: ["offers", "attributes", "summaries"],
            // },
            ...
            // query: {
            //     marketplaceIds: ["ATVPDKIKX0DER"],
            //     interval:'2022-01-01T00:00:00-07:00--2023-12-31T20:00:00-07:00',
            //     granularity: 'Total'
            // }
            ...
            // query: {
            //     "MarketplaceIds": "ATVPDKIKX0DER",
            //     "CreatedAfter": "2020-01-11T00:00:00.000Z",
            //     "NextToken": this.nextToken,
            //     "includedData": ['attributes', 'summaries'],
            // },
            // method: 'GET',

            // path: '/products/fees/v0/items/B0002JKQ9O/feesEstimate',
            // method: 'POST',
            // query: {
            //     "MarketplaceIds": "ATVPDKIKX0DER",
            //     "CreatedAfter": "2020-01-11T00:00:00.000Z",
        ...
        this._authService
            ...
            .subscribe(
                (respData: any) => {
                    this.nextToken = respData?.result.NextToken;
    ...
    getOrdersForGraphAmazon() {
        let endpoint = '/user-walmart/get-orders-data?startDate=' + this.startDate + '&endDate=' + this.endDate + '&platform=amazon';
    ...
    getOrdersForGraphWalmart() {
        let endpoint = '/user-walmart/get-orders-data?startDate=' + this.startDate + '&endDate=' + this.endDate + '&platform=walmart';
    ...
    getOrders() {
        ...
        let endpoint = '/user-amazon/get-analytics?page=' + (parseInt(this.currentPage) + 1) + '&pageSize=' + this.pageSize + '&startDate=' + this.startDate + '&endDate=' + this.endDate + filters;
        ...
        setTimeout(() => {
            if ((this.amazonOrderData || this.amazonOrderData?.length == 0) && (this.walmartOrderData || this.walmartOrderData?.length == 0)) {
                ...
                var option = {
                    ...
                    tooltip: {
                        backgroundColor:'#d1e0fb',
                        formatter: (res: any) => {
                            return `<div style="margin: 0px 0 0;line-height:1; padding:5px;">
                                        <div style="margin: 0px 0 0;line-height:1;">${res.marker}
                                            <span style="font-size:15px;color:#555;font-weight:400;margin-left:2px">${res.name}</span>
                                            <span style="float:right;margin-left:20px;font-size:15px;color:#555;font-weight:900">$${this.decimalpipe.transform(res.data, '1.2-2')}</span>
                    ...
                    option = {
                        ...
                        tooltip: {
                            ...
                            formatter: (res: any) => {
                              ...
                              <div style="margin: 0px 0 0;line-height:1;">${res.marker}
                                <span style="font-size:15px;color:#555;font-weight:400;margin-left:2px">${res.name}</span>
                                <span style="float:right;margin-left:20px;font-size:15px;color:#555;font-weight:900">$${this.decimalpipe.transform(res.seriesName == 2023 ? res.value[1] : res.value[2], '1.2-2')}</span>
                                <div style="clear:both"></div>
                              </div>
                              <div style="clear:both"></div>
                            </div>`
    ...
    getItems() {
        let endpoint = '/user-walmart/get-inventory-data';
        this._authService.setLoader(true);
        this._authService.sendRequest('GET', endpoint, '').subscribe((respData: any) => {
